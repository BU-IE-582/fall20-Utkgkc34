...
dataset <- read.csv('https://www.football-data.co.uk/mmz4281/1920/E0.csv', header = TRUE)

home_goals <- dataset$FTHG

away_goals <- dataset$FTAG

hist(home_goals, main = 'Histogram of Home Teams Goals', xlab = 'Home Goals', ylab = 'Number of Games', ylim = c(0 ,250))

yfith <- dpois(0:7, mean(home_goals))*250

lines(0:7, yfith)

hist(away_goals, main = 'Histogram of Away Team Goals', xlab = 'Away Goals', ylab = 'Number of Games', ylim = c(0, 250))

yfita <- dpois(0:6, mean(home_goals))*250

lines(0:6, yfita)

goal_difference <- home_goals- away_goals

hist(goal_difference, main = 'Histogram of Goal Differences', xlab = 'Goal Difference', ylab = 'Number of Games', xlim = c(-10, 10))
